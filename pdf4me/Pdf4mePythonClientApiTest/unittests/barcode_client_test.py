...
class BarcodeClientTest(unittest.TestCase):
    ...
    def test_barcode_action_throws_exception(self):
        # prepare args
        read_barcodes = self.create_read_barcode()
        read_barcodes.barcode_action = None
    ...
    def test_barcode_doc_length(self):
        ...
        read_barcodesd_pdf = len(res['document']['doc_data'])

        self.assertTrue(self.check.below_not_zero(read_barcodesd_pdf, original_length))
    ...
    def test_barcode_by_profile_doc_length(self):
        ...
        # validation
        original_length = self.test_files.pdf_length
        read_barcode_action_pdf = self.check.get_doc_base64_length(res)

        self.assertTrue(self.check.below_not_zero(read_barcode_pdf, original_length))
